...
as they are a non-uniform size, unlike numbers -- so we need a table
of symbols now as well as well as the local value index table becoming
a symbol table index to value index table....  *Keeping up?*
...
we might use the "constant index" -- so, arguably a :samp:`ci:{x}`.
The constant table is a big table of all the constants (*no,
seriously?*) that the evaluator has seen.  Symbols are unique and
``n`` is the same symbol whether it is in module ``m1`` or ``m2``.
...
symbol table to recover the original free variable name.

However, we only need to lookup the value index once, after which it
will always keep the same value index in the VM.  We can play another
little trick.  The byte code has :samp:`#ci:{n}` embedded in it.
Suppose we always go straight to the value index table.  We can
differentiate between the first and subsequent iterations by
initialising the index here to :samp:`{-n}`.
